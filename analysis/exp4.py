...
parser.add_argument("--cross_validate", type=int, default=0, help="Whether to do CV")
parser.add_argument("--n_folds", type=int, default=5, help="If do CV, the number of folds")
parser.add_argument("--cv_seed", type=int, default=0, help="If do CV, the random seed to partition CV training datasets")
parser.add_argument("--modelfullname_combidx", type=int, default=0, help="Which model to fit")
...
modelfullname_combidx = args.modelfullname_combidx

def exp4(cross_validate=0, n_folds=5, cv_seed=0, modelfullname_combidx = 0):
    ...
    piecewise_curve = lambda t, theta: (theta[:,0] + theta[:,1]*t)*(t<=(1/theta[:,1])) + ((1-theta[:,2])/(1/theta[:,1]-10)*t + (1/theta[:,1]*theta[:,2]-10)/(1/theta[:,1]-10) )*(t>(1/theta[:,1]))
    dt=1  #1
    ...
    theta_mean = 0
    theta_nu = 0.2
    condition_left_offsets = np.array([8, 2]) # Offsets for the seplate and sepearly conditions.
    ...
    def logistic_growth_group(ts, traj_assymtotes, left_offset):
        ...
        new_theta[:,1] = 1/(left_offset+np.squeeze(traj_assymtotes)) # Increasing portion slope
        new_theta[:,2] = 0.5+0.2*np.squeeze(traj_assymtotes)  # Slope
        return piecewise_curve(ts, new_theta)
    ...
    # 2) Load the datasets for all models
    # Get mTurk IDs
    filename = os.path.join(data_path,"exp4_data")
    with open(filename+'.pickle', 'rb') as handle:
        b = pickle.load(handle)
    mturk_IDs = b["mturk_IDs"]
    data = b["data"]
    [n_conds, n_subj, n_runs, n_timepts, _] = data.shape


    if(cross_validate):
        # CV dataset partition: Loop over fake datasets to fit models on.
        cross_val_setidxs_allsubjs = cross_validation_dataset_partition(data[:,:,:,:,1], n_folds, cv_seed)
        # Print length of train and test dataset for a particular subj's particular fold.
        print("Human dataset loaded and CV-partitioned: n_folds="+str(n_folds)+", seed="+str(cv_seed))
    else: 
        n_folds = 1
        print("Human datasets loaded, not doing CV.")
    ...
    # Models to be fitted
    model_fullname = model_fullnames[modelfullname_combidx]
        ...
        for model_idx in [0]:
            ...
            for subj in range(n_subj): # Just Subject 0.
                ...
                def task(init):
                    ...
                    if(cross_validate):
                        target = lambda theta: crossentropy_loss(theta, data_subj=data_subj, ts=ts, T_ends=T_ends, model=model, include_offset_param=include_offset_param, cross_validate=cross_validate, cross_validate_timepts=cross_val_setidxs[fold][0])
                    else:
                        target = lambda theta: crossentropy_loss(theta, data_subj=data_subj, ts=ts, T_ends=T_ends, model=model, include_offset_param=include_offset_param, cross_validate=cross_validate)
                ...
                if(cross_validate):
                    filename = os.path.join(fits_path, "exp4_fits_cv_temp__d_human__m_"+model_fullname)
                    #filename = "exp4_fits_temp"
                    ...
                    filename = os.path.join(fits_path, "exp4_fits_temp__d_human__m_"+model_fullname)
                    a = {'params_init':params_init,'params_fitted':params_fitted, 'crossentropy_fitted':crossentropy_fitted, 'model':model, 'model_include_offset_param':include_offset_param, 'model_fullname':model_fullname, 'total_time':total_time, 'mturk_IDs': mturk_IDs, 'data_dims': ["condition (0 is separate late)","subject","run","timestep (Month 0 to 9)","data (current trajectory height, current decision)"], 'data': data}
            ...
            # Save fitted model
            if(cross_validate):
                filename = os.path.join(fits_path, "exp4_fits_cv__d_human__m_"+model_fullname)
                ...
                filename = os.path.join(fits_path, "exp4_fits__d_human__m_"+model_fullname)
                a = {'params_init':params_init,'params_fitted':params_fitted, 'best_params_fitted':best_params_fitted, 'crossentropy_fitted':crossentropy_fitted, 'model':model, 'model_include_offset_param':include_offset_param, 'model_fullname':model_fullname, 'total_time':total_time, 'mturk_IDs': mturk_IDs, 'data_dims': ["condition (0 is separate late)","subject","run","timestep (Month 0 to 9)","data (current trajectory height, current decision)"], 'data': data}
            with open(filename+'.pickle', 'wb') as handle:
                pickle.dump(a, handle, protocol=pickle.HIGHEST_PROTOCOL)
    if(cross_validate):
        print("CV model fits have ended.")
    else:
        print("Full model fits have ended.")

exp4(cross_validate, n_folds, cv_seed, modelfullname_combidx)